...
class Config:
    ...
    moco_dim: int = 128
    moco_k: int = 4096             # velicina queue-a
    moco_m: float = 0.999          # EMA momentum za key encoder
    temperature: float = 0.2
    device: str = "cuda" if torch.cuda.is_available() else "cpu"
    log_path: str = "moco_pretrain_log.csv"
...
class MoCo(nn.Module):
    """
    Minimalna verzija MoCo-v2 za CIFAR-10.
    """
    ...
    def forward(self, im_q, im_k):
        ...
        """
        # query branch
        q = self.encoder_q(im_q)           # [B, feat]
        q = self.projector_q(q)            # [B, dim]
        q = F.normalize(q, dim=1)

        with torch.no_grad():
            self._momentum_update_key_encoder()
            # shuffle BN (za multi-gpu bi trebalo, ovdje ignoriramo)

            k = self.encoder_k(im_k)
            k = self.projector_k(k)
            k = F.normalize(k, dim=1)

        # kontrastivni logits: q protiv k (poz) + queue (neg)
        # q: [B, dim]
        # k: [B, dim]
        # queue: [dim, K]
    ...
    def get_encoder(self):
        """
        Vrati samo encoder_q (backbone) za linear eval.
        """
...
def pretrain_moco():
    train_loader = get_moco_dataloader()
    model = MoCo(dim=CFG.moco_dim, K=CFG.moco_k, m=CFG.moco_m, T=CFG.temperature)
    ...
    for epoch in range(1, CFG.epochs_pretrain + 1):
        ...
        print(f"[MoCo Pretrain] Epoch [{epoch}/{CFG.epochs_pretrain}] Loss: {avg_loss:.4f}")
    ...
    torch.save(encoder.state_dict(), "moco_encoder.pth")
    print(">> Saved MoCo encoder to moco_encoder.pth")
