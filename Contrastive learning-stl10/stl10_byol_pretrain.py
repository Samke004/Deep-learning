...
class Config:
    ...
    log_path: str = "stl10_byol_pretrain_log.csv"
...
def get_byol_dataloader():
    dataset = datasets.STL10(
        ...
        split="unlabeled",
        download=True,
        transform=BYOLTransform(image_size=96),
    )
...
class BYOL(nn.Module):
    def __init__(self, dim=256, m_ema=0.996):
        super().__init__()
        self.m_ema = m_ema
...
def pretrain_byol():
    ...
    for epoch in range(1, CFG.epochs_pretrain + 1):
        ...
        print(f"[STL10 BYOL] Epoch [{epoch}/{CFG.epochs_pretrain}] Loss: {avg_loss:.4f}")
    ...
    torch.save(encoder.state_dict(), "stl10_byol_encoder.pth")
    print(">> Saved STL10 BYOL encoder to stl10_byol_encoder.pth")
